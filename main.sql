...
CREATE TABLE icard(
  ...
  fines NUMBER,
  CONSTRAINT icard_PK PRIMARY KEY (icardID));
...
CREATE TABLE fees(
  ...
  returnDate DATE,
  CONSTRAINT fees_PK PRIMARY KEY (icardID,itemID));
...
FOREIGN KEY (icardNumber)
REFERENCES icard(icardID);

ALTER TABLE emp
ADD CONSTRAINT emp_FK_icard
FOREIGN KEY (icardNumber)
REFERENCES icard(icardID);

ALTER TABLE emp
ADD CONSTRAINT emp_FK_Branch
...
ALTER TABLE fees
ADD CONSTRAINT fees_FK_icard
FOREIGN KEY (icardID)
REFERENCES icard(icardID);

ALTER TABLE fees
ADD CONSTRAINT fees_FK_Book
FOREIGN KEY (itemID)
REFERENCES Book(bookID);

ALTER TABLE fees
ADD CONSTRAINT fees_FK_Video
FOREIGN KEY (itemID)
REFERENCES Video(videoID);


--INSERTS--
INSERT INTO icard VALUES (101,'A',0);
INSERT INTO icard VALUES (102,'A',0);
INSERT INTO icard VALUES (103,'A',0);
INSERT INTO icard VALUES (104,'A',0);
INSERT INTO icard VALUES (105,'A',0);
INSERT INTO icard VALUES (106,'A',0);
INSERT INTO icard VALUES (107,'B',50);
INSERT INTO icard VALUES (108,'B',10);
INSERT INTO icard VALUES (109,'B',25.5);
INSERT INTO icard VALUES (110,'B',15.25);
INSERT INTO icard VALUES (151,'A',0);
INSERT INTO icard VALUES (152,'A',0);
INSERT INTO icard VALUES (153,'A',0);
INSERT INTO icard VALUES (154,'A',0);
INSERT INTO icard VALUES (155,'A',0);
...
INSERT INTO emp VALUES (211, 'ROSS', 'HIS HOUSE', 671671671, 'ross123', 'ro11', 1200, 'ARCHEOLOGY', 551);
INSERT INTO emp VALUES (212, 'CHANDLER', 'OUR HEARTHS', 688688688, 'chandler123', 'chand12', 1150.50, 'ARCHEOLOGY', 552);
INSERT INTO emp VALUES (213, 'JOEY', 'LITTLE ITAYLY', 628628628, 'joey123', 'jo13', 975.75, 'ARCHEOLOGY', 553);
INSERT INTO emp VALUES (214, 'VICTOR', 'SANTA FE', 654321987, 'victor123', 'vic14', 2200, 'COMPUTING', 554);
INSERT INTO emp VALUES (215, 'JAIRO', 'ARMILLA', 698754321, 'jairo123', 'ja15', 2200.50, 'CHEMISTRY', 555);
...
INSERT INTO fees VALUES (101, 'B2A123', '10-05-2018', '20-05-2018');
INSERT INTO fees VALUES (102, 'B1Z123', '10-05-2018', '25-05-2018');
INSERT INTO fees VALUES (104, 'V1JA15', '01-05-2018', '21-05-2018');
INSERT INTO fees VALUES (105, 'V1DI00', '02-05-2018', '25-05-2018');
INSERT INTO fees VALUES (154, 'B1L321', '04-05-2018', '26-05-2018');
INSERT INTO fees VALUES (155, 'V1CH16', '29-04-2018', '29-05-2018');
...
--emp-
CREATE OR REPLACE PROCEDURE loginemp_library(user IN VARCHAR2, pass IN VARCHAR2)
IS
  passAux emp.password%TYPE;
  incorrect_password EXCEPTION;
BEGIN
  SELECT password INTO passAux
  FROM emp
...
SET SERVEROUTPUT ON;
DECLARE
  user emp.username%TYPE;
  pass emp.password%TYPE;
BEGIN
  user := &Username;
  pass := &Password;
  login_emp_library(user,pass);
END;
...
IS
  ...
  feesed number := 0;
BEGIN
  SELECT icardnumber INTO auxicard
  FROM customer
  WHERE customerid LIKE custoID;
  
  SELECT COUNT(*) INTO feesed
  FROM fees
  WHERE fees.icardid LIKE auxicard;
  
  DBMS_OUTPUT.PUT_LINE('The user icard is ' || auxicard);  
  IF (feesed > 0) THEN
    SELECT fees.itemid INTO auxItem
    FROM fees,icard
    WHERE icard.icardid = fees.icardid
    AND icard.icardid LIKE auxicard;    
    
    DBMS_OUTPUT.PUT_LINE('The user has ' || auxItem || ' feesed');
  ELSE    
    DBMS_OUTPUT.PUT_LINE('This user has no feess'); 
  END IF;
  
  SELECT fines INTO auxFines
  FROM icard
  WHERE icardid LIKE auxicard;
...
--emp--
CREATE OR REPLACE PROCEDURE empAccount_library(emploID IN emp.empid%TYPE)
IS
  auxicard NUMBER;
  auxFines NUMBER;
  auxItem VARCHAR(6);
...
SET SERVEROUTPUT ON;
DECLARE
  emploID emp.empid%TYPE;
BEGIN
  emploID := &emp_ID;
  empAcount_library(emploID);
END;



--4--
CREATE OR REPLACE PROCEDURE feesItem_library(auxicard IN NUMBER, auxItemID IN VARCHAR2, itemType IN VARCHAR2, auxDate IN DATE)
IS
  ...
  IF statusAux LIKE 'A' THEN
    IF itemType LIKE 'book' THEN
      ...
      IF itemStatus LIKE 'A' THEN
        ...
        INSERT INTO fees
        VALUES (auxicard,auxItemID,sysdate,auxDate);
        DBMS_OUTPUT.PUT_LINE('Item ' || auxItemID || ' feesed');
      ELSE
        DBMS_OUTPUT.PUT_LINE('The item is already feesed')
      END IF;
...
BEGIN
  ...
  auxItemID := &ID_Item;  
  auxDate := &Return_date;
  feesItem_library(auxicard,auxItemID,itemType,auxDate);
END;
...
--5--
CREATE OR REPLACE PROCEDURE payFines_library(auxicard IN icard.icardid%TYPE, money IN NUMBER)
IS
  finesAmount NUMBER;
  total NUMBER;
BEGIN
  ...
  ELSE
    ...
    DBMS_OUTPUT.PUT_LINE('YOU WILL NEED TO PAY ' || total || ' MORE DOLLARS TO UNLOCK YOUR icard');
...
--emp--
CREATE OR REPLACE PROCEDURE updateInfoEmp_library(auxemp IN emp.empid%TYPE, pNumber NUMBER, address VARCHAR2, newPass VARCHAR2, newPayCheck NUMBER,
newBranch VARCHAR2)
IS
BEGIN
  UPDATE emp
  SET phone = pNumber, customeraddress = address, password = newPass, paycheck = auxemp, branchname = newBranch
  WHERE empid = auxemp;
END;

SET SERVEROUTPUT ON;
DECLARE
  auxemp emplouee.empid%TYPE;
  pNumber NUMBER;
  address VARCHAR2;
  newPass VARCHAR2;
  newPayCheck NUMBER;
...
BEGIN
  ...
  updateInfoemp_library(auxCustomer,pNumber,address,newPass,newPayCheck,newBranch);
END;
...
auxPass IN VARCHAR2, auxUserName IN VARCHAR2, auxicardNumber IN NUMBER)
IS
BEGIN
  INSERT INTO customer
  VALUES (auxCustomerId,auxName,auxCustomerAddress,auxPhone,auxPass,auxUserName,sysdate,auxicardNumber);
END;
...
BEGIN
  ...
  auxicardNumber := &icard_Numeber;
  addCustomer_library(auxCustomerId,auxName,auxCustomerAddress,auxPhone,auxPass,auxUserName,auxicardNumber);
END;
...
--8--
--CUSTOMER--
CREATE OR REPLACE TRIGGER addicardCusto_library
AFTER INSERT
ON customer
FOR EACH ROW
DECLARE
BEGIN
  INSERT INTO icard
  VALUES (:new.icardnumber,'A',0);
  
  DBMS_OUTPUT.PUT_LINE('icard created');
END;

--emp--
CREATE OR REPLACE TRIGGER addicardEmp_library
AFTER INSERT
ON emp
FOR EACH ROW
DECLARE
...
BEGIN
  ...
  IF auxfeesed > 0 THEN
    ...
    END IF;
  ELSE
    DBMS_OUTPUT.PUT_LINE('This item is not feesed at the moment');
  END IF;
  EXCEPTION WHEN no_data_found THEN 
...
DECLARE
  ...
  auxDeby NUMBER;
BEGIN  
  SELECT icardid, itemid INTO auxicardID, auxItemID
  FROM fees
  WHERE icardid LIKE :old.icardid;
...
DELETE FROM icard WHERE icardid LIKE 112;
SELECT * FROM icard;
...
IS
  ...
  custoicard NUMBER;
BEGIN
  SELECT name,customeraddress,phone,username,datesignup,icardnumber
  INTO custoName, custoAdd, custoPhone, userNaM, custoDate, custoicard
  FROM customer
  ...
  DBMS_OUTPUT.PUT_LINE('icard NUMBER: ' || custoicard);
  DBMS_OUTPUT.PUT_LINE('------------------------------------------');
...
BEGIN 
   ...
   dbms_output.put_line('DIRECTOR ID: '|| director.empid); 
   dbms_output.put_line('--------------------------------------------' ); 
